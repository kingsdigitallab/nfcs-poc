...
- **SampleDataSource** — loads pre-packaged collection files from public/fixtures/ via a curated manifest. Pick a named package (e.g. "Stonehenge") and tick individual files; fetches and extracts them as FileRecord[]. Same five typed output handles as LocalFolderSource (results/pdf/xml/text/image). Unlike LocalFolderSource it **has a runner and participates in Run All** — ideal for offline demos and saved example workflows.
...
- **QuickNote** — display-only annotation node with a pass-through output handle; no runner. Three modes (selector: Note · Structured · Score):
  - *Note* — free-text note per record written to \`_note\`; a field picker lets you read any upstream field in full while annotating. Notes are shared with TableOutput via a notesStore.
  - *Structured* — configure a set of fields (\`{key, label}\`) once; per-record form assembles a clean JSON gold-standard object into a target field (default \`_note\`) without hand-typing JSON. The JSON flows into the Evaluator's reference token (\`{{__reference}}\`). Mutual exclusion: a structured entry clears any prose note for that record.
  - *Score* — constrained per-criterion human scoring. Configure criteria (\`{key, label, scale: number[]}\`) and a target field (default \`human_score\`). Clicking a scale value writes \`human_score\` (object keyed by criterion + flat \`human_c*\` primitive keys + a \`labels\` map for downstream display). Optional prose reason per criterion. "Clear all" with confirm. Human counterpart to the Evaluator's LLM judge.
- **Evaluator** — LLM-as-judge, runnable. Scores a \`candidateField\` against a \`referenceField\` on the **same record** using an ARC model at **temperature 0** for repeatability. Per-criterion scoring only (never one aggregate score). Built-in rubric presets (Extraction agreement, Interpretive agreement, Rubric-from-note). Template tokens: \`{{__reference}}\`, \`{{__candidate}}\`. Writes \`record.eval = {scores, reasons, raw, status}\` + flat \`eval_c*\` columns. Shows judge-vs-human agreement readout when a human score field is present. Tolerant JSON parsing — never throws; sets status to \`parse_error\` on bad output. Requires KCL API key.
...
- **ComparisonReport** — read-only evaluation surface; no runner, skipped by Run All. Map five report roles to actual upstream fields via dropdowns: *original*, *note/reference*, *response/candidate*, *judge score*, *human score*. Renders per-record cards plus an aggregate agreement summary matrix. Human criterion labels (from QuickNote Score mode's \`labels\` map) appear in the summary. Supports double-click full-screen "projector-legible" view.
...
- **TableOutput expand namespaces** reveals all service-specific fields (ariadne.*, hsds.*, bodleian.*, europeana.*) as flat columns — needed for cross-service comparison.
...
- **ADS is unavailable** — both ADSSearchAdvanced and ADSLibrary are blocked by Cloudflare. Use **ARIADNESearch** (Contributor = "Archaeology Data Service") or **HSDSSearch** for UK heritage data instead.
- **Evaluation pipeline**: source → KingsInference (generates candidate response) → QuickNote in *Note* or *Structured* mode (human writes reference) + QuickNote in *Score* mode (human rates criteria) → **Evaluator** (LLM judge scores candidate vs reference) → **ComparisonReport** (side-by-side cards + agreement matrix). QuickNote nodes are display-only and pass records through unchanged, so they can be placed anywhere in the chain.
- **Group node** — select any nodes on the canvas, then right-click → Group (or use the Group entry in the sidebar) to bundle them into a movable, labelled container. Groups are purely visual; they do not affect data flow.
