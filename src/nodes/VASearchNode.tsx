...
const WIRABLE_ROWS = [
  { handleId: 'query', dataKey: 'inlineQuery', label: 'query', placeholder: 'e.g. ceramics', rowIndex: 0 },
...
const STATUS_BORDER: Record<VAStatus, string> = {
  idle:    '#d1d5db',
  loading: '#3b82f6',
  success: '#22c55e',
  error:   '#ef4444',
...
export function VASearchNode({ id, data }: NodeProps) {
  const { updateNodeData, getNodes, getEdges: getEdgesSnap } = useReactFlow()
  const liveEdges = useEdges()
  const d = data as VASearchNodeData
  const [filtersOpen, setFiltersOpen] = useState(false)

  const fetchAll    = d.fetchAll ?? false
  const borderColor = STATUS_BORDER[d.status as VAStatus] ?? '#d1d5db'
  ...
  const handleRun = useCallback(
    () => nodeRunners.vaSearch(id, getNodes, getEdgesSnap(), updateNodeData),
  ...
  const activeFilterCount = [d.imagesOnly && 'img', d.yearFrom, d.yearTo, d.objectType].filter(Boolean).length
  ...
  return (
    <div style={{ ...styles.card, borderColor }}>
      ...
      <div style={styles.header}>
        <span style={styles.headerTitle}>Victoria &amp; Albert Museum</span>
        {d.statusMessage ? (
          <span style={{ ...styles.statusBadge, color: STATUS_BADGE[d.status as VAStatus] ?? '#9ca3af' }}>
      ...
      <div style={styles.body}>
        ...
        {filtersOpen && (
          <div style={styles.filterSection}>
            <label style={{ ...styles.checkLabel, marginBottom: 2 }} className="nodrag">
              <input
                type="checkbox"
                checked={!!d.imagesOnly}
                onChange={e => updateNodeData(id, { imagesOnly: e.target.checked })}
                style={{ marginRight: 5 }}
              />
              Images only
            </label>
            ...
            <div style={styles.filterRow}>
              <span style={styles.filterLabel}>Year from</span>
              <input
                style={styles.inlineInput}
                value={d.yearFrom || ''}
            ...
            <div style={styles.filterRow}>
              <span style={styles.filterLabel}>Year to</span>
              <input
                style={styles.inlineInput}
                value={d.yearTo || ''}
                onChange={set('yearTo')}
            ...
            {activeFilterCount > 0 && (
              <button
                ...
                onClick={() => updateNodeData(id, { imagesOnly: false, objectType: '', yearFrom: '', yearTo: '' })}
                className="nodrag"
              >
                ✕ Clear all filters
              </button>
        ...
        <div style={styles.fixtureControls}>
          ...
          {(d.status === 'success' || d.status === 'cached') && (
            <button
              ...
              title={`Download fixture: ${fixtureFilename('vaSearch', resolveFixtureQuery(id, liveEdges, getNodes(), d as Record<string, unknown>))}`}
              onClick={() => downloadAsFixture(id, 'vaSearch', resolveFixtureQuery(id, liveEdges, getNodes(), d as Record<string, unknown>))}
            >💾</button>
          )}
        </div>
        <button
...
const HEADER_COLOR  = '#9f1239'
const RUN_BTN_COLOR = '#be123c'
