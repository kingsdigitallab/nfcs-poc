...
const STATUS_BORDER: Record<HSDSStatus, string> = {
  idle:    '#d1d5db',
  loading: '#3b82f6',
  success: '#22c55e',
  error:   '#ef4444',
...
const STATUS_BADGE: Record<HSDSStatus, string> = {
  idle:    '#9ca3af',
  loading: '#93c5fd',
  success: '#86efac',
  error:   '#fca5a5',
...
export function HSDSSearchNode({ id, data }: NodeProps) {
  const { updateNodeData, getNodes, getEdges: getEdgesSnap } = useReactFlow()
  const liveEdges = useEdges()
  const d = data as HSDSSearchNodeData
  const [filtersOpen, setFiltersOpen] = useState(false)

  const fetchAll    = d.fetchAll ?? false
  const borderColor = STATUS_BORDER[d.status as HSDSStatus] ?? '#d1d5db'
  ...
  const handleRun = useCallback(
    () => runHSDSNode(id, getNodes, getEdgesSnap(), updateNodeData),
  ...
  return (
    <div style={{ ...styles.card, borderColor }}>
      ...
      <div style={styles.header}>
        <span style={styles.headerTitle}>HSDS Search</span>
        {d.statusMessage ? (
          <span style={{ ...styles.statusBadge, color: STATUS_BADGE[d.status as HSDSStatus] ?? '#9ca3af' }}>
...
const HEADER_COLOR  = '#134e4a'  // teal-900 — heritage data service
const RUN_BTN_COLOR = '#0f3d3a'
