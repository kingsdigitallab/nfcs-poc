...
const profiles: SourceProfile[] = [
...

  {
    ...
    limitations: 'Dataset-level metadata only — not object-level detail. Coordinate coverage varies; many records lack precise geopoints. Temporal metadata is often period labels rather than point dates. Filtered via Vite proxy (no Cloudflare, but may require proxy for production deployment).',
    coreFields: ['title', 'description', 'date', 'subject', 'country', 'decimalLatitude', 'decimalLongitude'],
    fields: [
      { path: 'title',                  label: 'Title',             description: 'Title of the heritage dataset or resource', type: 'string', mapsToCore: 'title' },
      { path: 'description',            label: 'Description',       description: 'Abstract or summary description of the resource', type: 'string', mapsToCore: 'description' },
      { path: 'date',                   label: 'Date Issued',       description: 'Issue or creation date of the dataset record', type: 'string', mapsToCore: 'date' },
      { path: 'subject',                label: 'Subjects',          description: 'Controlled subject terms merged from nativeSubject and derivedSubject arrays', type: 'string[]', mapsToCore: 'subject' },
      { path: 'country',                label: 'Country',           description: 'Country or nation (England, Scotland, Wales, Northern Ireland)', type: 'string', mapsToCore: 'country' },
      { path: 'decimalLatitude',        label: 'Latitude',          description: 'Site centroid latitude — not always present', type: 'number', mapsToCore: 'decimalLatitude' },
      { path: 'decimalLongitude',       label: 'Longitude',         description: 'Site centroid longitude — not always present', type: 'number', mapsToCore: 'decimalLongitude' },
      { path: 'periodName',             label: 'Period Name',       description: 'Archaeological or historic period label(s)', type: 'string', mapsToCore: 'periodName' },
      { path: 'periodStart',            label: 'Period Start',      description: 'Start of temporal coverage (ISO date or year)', type: 'string', mapsToCore: 'periodStart' },
      { path: 'periodEnd',              label: 'Period End',        description: 'End of temporal coverage (ISO date or year)', type: 'string', mapsToCore: 'periodEnd' },
      { path: 'spatialCoverage',        label: 'Place Name',        description: 'First place name from spatial metadata', type: 'string', mapsToCore: 'spatialCoverage' },
      { path: 'hsds.landingPage',       label: 'Landing Page',      description: 'URL to the resource on the contributing organisation\'s website', type: 'url', notes: 'More reliable than _sourceUrl for deep-linking' },
      { path: 'hsds.contributor',       label: 'Contributors',      description: 'Names of contributing organisations (e.g. Historic England, HES, Cadw)', type: 'string[]', notes: 'Use to scope results to a specific national body' },
      { path: 'hsds.publisherName',     label: 'Publisher',         description: 'Primary publisher of the dataset', type: 'string' },
      { path: 'hsds.hasType',           label: 'Resource Type',     description: 'High-level resource type label (e.g. Site, Collection, Dataset)', type: 'string' },
      { path: 'hsds.nativeSubject',     label: 'Native Subject',    description: 'Subject terms in the contributor\'s own vocabulary (raw, not normalised)', type: 'string[]' },
      { path: 'hsds.derivedSubject',    label: 'AAT Subject',       description: 'Subject terms mapped to Getty Art and Architecture Thesaurus (AAT)', type: 'string[]', notes: 'Use for cross-source subject matching with ARIADNE' },
      { path: 'hsds.ariadneSubject',    label: 'ARIADNE Subject',   description: 'ARIADNE portal subject classification', type: 'string[]' },
      { path: 'hsds.dataType',          label: 'Data Type',         description: 'Data type labels describing the nature of the dataset', type: 'string[]' },
      { path: 'hsds.temporal',          label: 'Temporal (raw)',    description: 'Full structured temporal coverage array (from/until/periodName/uri)', type: 'string' },
      { path: 'hsds.allPlaceNames',     label: 'All Place Names',   description: 'All place names extracted from the spatial array', type: 'string[]' },
      { path: 'hsds.accessPolicy',      label: 'Access Policy',     description: 'Access policy for the dataset', type: 'string' },
      { path: 'hsds.accessRights',      label: 'Access Rights',     description: 'Rights statement for accessing the dataset', type: 'string' },
      { path: 'hsds.modified',          label: 'Last Modified',     description: 'Date the record was last modified', type: 'string' },
    ],
    correspondences: [
      { field: 'title',                otherSource: 'ariadne',  otherField: 'title',               relationship: 'direct',           notes: 'Direct equivalent — both are dataset titles on the same ARIADNE backbone' },
      { field: 'hsds.derivedSubject',  otherSource: 'ariadne',  otherField: 'ariadne.derivedSubject',relationship: 'direct',          notes: 'Both mapped to Getty AAT — directly cross-searchable' },
      { field: 'hsds.nativeSubject',   otherSource: 'ariadne',  otherField: 'ariadne.nativeSubject', relationship: 'similar',         notes: 'Both native vocabularies but from different contributing institutions' },
      { field: 'hsds.contributor',     otherSource: 'ariadne',  otherField: 'ariadne.contributor',  relationship: 'direct',           notes: 'ARIADNE Contributor = "Archaeology Data Service" overlaps with HSDS contributor records from ADS' },
      { field: 'decimalLatitude',      otherSource: 'gbif',     otherField: 'decimalLatitude',      relationship: 'join-candidate',   notes: 'Co-locate heritage sites with biodiversity occurrence records' },
      { field: 'country',              otherSource: 'europeana', otherField: 'europeana.country',   relationship: 'similar',          notes: 'Both carry geographic scope — country vs institution country' },
      { field: 'periodName',           otherSource: 'ariadne',  otherField: 'ariadne.period',       relationship: 'direct',           notes: 'Period labels — directly comparable, same vocabulary' },
    ],
  },
