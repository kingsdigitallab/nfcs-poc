...
| **FieldDistributionNode** | Faceted bar chart of value frequencies for any field in upstream records. Pick a field from the dropdown — array-valued fields (`subject`, `country`, `creator`) are expanded so each element is tallied individually (toggle **expand arrays** to disable). Bars are sorted by count descending and capped at a configurable **Top N** (default 20). Displays count and percentage on each bar. Click any bar to toggle it as a filter: selected bars highlight in green, unselected bars dim. Matching records are stored reactively on the output handle — connect it to TableOutputNode, FilterTransformNode, or any other process node. When nothing is selected, all records pass through unchanged. Hover a bar for a preview hint showing whether a click will add or remove the filter. A status bar shows how many records currently match the selection with a **✕ clear** button. Updates live as upstream records change. |
...
ParamNode ─┐
  ...
  [source] ──► URLFetchNode ──► HTMLSectionNode ──► OllamaFieldNode ──┤
               [source] ──► XMLSectionNode ──────────────────── ┤
                                                      ...
                                                      CitationNode               │
                                                      QuickViewNode              ▼
                                                      JSONOutputNode  MapOutputNode ◄── LocalFolderSourceNode (GIS)
                                                      TimelineOutputNode
                                                      OllamaOutputNode
